...
public class MyController {
	...
	public ResponseEntity<?> addProduct(@ModelAttribute PostProductDAO postProductDAO) throws Exception {
		...
		var file = postProductDAO.getFiel();
	...
	public ResponseEntity<?> updateProduct(@ModelAttribute PutProductDAO putProductDAO) throws Exception {
		...
		if (p.isPresent()) {
			...
			if (putProductDAO.getFiel() != null) {
				String uploadDir = new File("myApp/static").getAbsolutePath();
				File dir = new File(uploadDir);
				if (!dir.exists()) {
					dir.mkdirs();
				}
				var file = putProductDAO.getFiel();
				String uniqueName = UUID.randomUUID() + "_" + file.getOriginalFilename();
				String filePath = Paths.get(uploadDir, uniqueName).toString();
	...
	public ResponseEntity<?> deleteById(@PathVariable("id") Integer id){
		...
		if(pro.isPresent()) {
			...
			return ResponseEntity.status(HttpStatus.ACCEPTED).body(Map.of("message","Product id = "+id+" Has been delete!"));
		}
		return ResponseEntity.status(400).body(Map.of("message","Product id = "+id+" not found "));
	}
